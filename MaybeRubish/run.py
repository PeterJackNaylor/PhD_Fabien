...
from DataToLMDB import MakeLMDB, MakeDataLikeFCN
...
if __name__ == "__main__":
    ...
    if options.wd is None:
        options.wd = '/home/naylor/Documents/FCN_wd'
    ...
    if create_dataset:
        ...
        MakeDataLikeFCN(options.rawdata, output_dir, transform_list)

    data_folder = output_dir
    output_folder = options.wd + options.cn
     
    if create_net:
        data_train = os.path.join(data_folder, "train")
        data_test = os.path.join(data_folder, "test")
   
        FCN32.make_net(data_train, data_test)

    solver_path = options.wd + options.cn + "/solver.protoxt"
    
    if create_solver:
        
        name_solver = solver(solver_path, test_net_path=None, base_lr=0.001, out_snap = options.wd + options.cn + "/temp_snapshot")
