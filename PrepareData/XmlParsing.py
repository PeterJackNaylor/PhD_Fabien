...
def CreateFolder(data_file, options):
    for string in data_file["Path_PNG"]:
        ...
        num = ending.split('Slide_')[-1]
        #pdb.set_trace()
        string = os.path.join(options.output, "Slide_{:02d}").format(int(num))
        ...
        if options.output_diff != "None":
            string = os.path.join(options.output_diff, "Slide_{:02d}").format(int(num))
            if not os.path.isdir(string):
                print "Creating folder: {}".format(string)
            CheckOrCreate(string)
...
def SaveName(options):
    return os.path.join(options.output, "GT_{:02d}/{:02d}_{}.png")
def SaveDiffName(options):
    return os.path.join(options.output_diff, "Slide_{:02d}/{:02d}_{}.png")
