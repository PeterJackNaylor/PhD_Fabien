...
class FCNdatalayerOnFLy(FCNdatalayer):

    def __init__(self, transform):
        FCNdatalayer.__init__(self)
    ...
    def setup(self, bottom, top):
        ...
        if self.random:
            random.seed(self.seed)
            self.idx = random.randint(0, len(self.indices) - 1)
            self.t_idx = random.randint(0, len(self.transform) - 1)

    def reshape(self, bottom, top):
        # load image + label image pair
        self.data = self.load_image(
            self.indices[self.idx], self.transform[self.t_idx])
    ...
    def forward(self, bottom, top):
        ...
        if self.random:
            ...
            if self.t_idx == len(self.transform):
                self.t_idx = 0
                self.idx += 1
                if self.idx == len(self.indices):
                    self.idx = 0
