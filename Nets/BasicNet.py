...
def DeconvBnRelu(bottom, nout, ks=3, pad=0, stride=1, weight_filler=Gaussian_fil, bias_filler=Constant_fil):
...
def DeconvReConvReConvRe(bottom1, val, deconv_out=None):
    ...
    deconv = Deconv(bottom1, deconv_out, 1, 2, 2,
                    xavier, Constant_fil)
    relu1 = Relu(deconv)
    conv2, relu2 = ConvRelu(relu1, val)
    conv3, relu3 = ConvRelu(conv2, val)
...
def DeconvReCropConcatConvReConvRe(bottom1, bridge2, val, deconv_out=None):
    ...
    conv2, relu2 = ConvRelu(concat, val)
    conv3, relu3 = ConvRelu(conv2, val)
